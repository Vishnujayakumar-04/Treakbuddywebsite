...
const CATEGORY_IMAGES: Record<string, string> = {
    ...
    restaurants: '/assets/food-dining/villa-shanti/photo-1.jpg',
    nature: '/assets/nature/botanical-gardens/photo-1.jpg',
    adventure: '/assets/adventures/surfing-lessons/photo-1.jpg',
    shopping: '/assets/shopping/sunday-market/photo-1.jpg',
    hotels: '/assets/hotels-stays/accord-puducherry/photo-1.jpg',
    nightlife: '/assets/nightlife-pubs/heist-nightlife/photo-1.jpg',
    theatre: '/assets/theatre/theather_cover_img.jpg',
};

const MARQUEE_IMAGES = [
    '/assets/beaches/promenade-beach/photo-1.jpg',
    '/assets/hotels-stays/villa-shanti/photo-1.jpg',
    ...
    '/assets/nature/pichavaram-mangroves/photo-1.jpg',
];
...
export default function CategoriesPage() {
    ...
    // Optimize search with Exact Match Priority
    let filteredCategories = categories;

    if (searchQuery.trim() !== '') {
        const q = searchQuery.toLowerCase().trim();

        // 1. Check for Exact Place or Category Names first
        const exactMatches = categories.filter(cat => {
            if (cat.name.toLowerCase() === q) return true;
            
            const placesInCat = getPlacesByCategory(cat.id);
            return placesInCat.some(place => place.name.toLowerCase() === q);
        });

        // 2. If an exact match exists, ONLY return those categories. 
        // Otherwise, fallback to broad fuzzy inclusion search.
        if (exactMatches.length > 0) {
            filteredCategories = exactMatches;
        } else {
