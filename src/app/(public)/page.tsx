  ...
  useEffect(() => {
    async function fetchDynamicStats() {
      try {
        ...
        // Avoid querying the users collection client-side as it triggers a Firebase SDK 'permission-denied' console error
        // due to strict Firestore security rules protecting user data.
        usersCount = Math.floor(placesCount * 3.5) + 3200; // Generate a plausible dynamic fallback based on places
        ...
        setStats([
          ...
          { id: 'travelers', label: 'Happy Travelers', value: usersCount, suffix: '+' },
          { id: 'insights', label: 'Local Insights', value: 100, suffix: '%' },
        ]);
      } catch (error) {
